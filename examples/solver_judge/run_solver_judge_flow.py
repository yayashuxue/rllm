...
def evaluate_results(results):
    ...
    for i, episode in enumerate(results[:3]):
        print(f"\nExample {i+1}:")
        print(f"Task: {episode.task['question'][:100]}...")
        print(f"Is Correct: {episode.is_correct}")
        
        # Find the judge's final step to get the selected solution and reward
        for agent_name, trajectory in episode.trajectories:
            if agent_name == "judge" and trajectory.steps:
                final_step = trajectory.steps[-1]
                print(f"Selected solution: {getattr(final_step, 'model_response', 'N/A')[:200]}...")
                print(f"Reward: {getattr(final_step, 'reward', 'N/A')}")
                break
...
if __name__ == "__main__":
    ...
    # Run the workflow
    print("Running solver-judge workflow on countdown tasks...")
