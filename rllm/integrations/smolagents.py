...
try:
    ...
    from smolagents.models import ChatMessage, ChatMessageStreamDelta, OpenAIServerModel, TokenUsage
...
{str(additional_args)}."""
        ...
        if self.return_full_result:
            from smolagents.memory import ActionStep, PlanningStep, TokenUsage, Timing
    ...
    async def _arun_stream(
        ...
        """
        import time
        from smolagents.memory import ActionStep, Timing, FinalAnswerStep
...
class RLLMOpenAIModel(SmolModel):
    ...
    def __init__(
        self,
        rollout_engine=None,
        application_id: str = "smolagent-rllm",
        ...
        **kwargs
        ...
        if not rollout_engine:
            raise ValueError(
                "rollout_engine is required for RLLMOpenAIModel. "
                "Pass an instance of rLLM's RolloutEngine."
            )
        ...
        **kwargs
        ...
        try:
            ...
            if SMOLAGENTS_AVAILABLE:
                # Try to estimate token usage (rough approximation)
                ...
                input_tokens = total_content_length * 1.3
                output_tokens = len(str(response).split()) * 1.3
                
                self._last_input_token_count = int(input_tokens)
                self._last_output_token_count = int(output_tokens)
            ...
            if SMOLAGENTS_AVAILABLE:
                ...
                raise AgentError(f"Model generation failed: {e}", agent_logger)
            else:
        ...
        try:
            ...
            if loop.is_running():
                ...
                return loop.run_until_complete(
                    self.generate_async(messages, stop_sequences, response_format, tools_to_call_from, **kwargs)
                )
