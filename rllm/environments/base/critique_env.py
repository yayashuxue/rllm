...
class CritiqueEnvironment(MultiTurnEnvironment):
    """
    A simple environment for multi-turn interactions between a solver and a critic.
    This is a special case of MultiTurnEnvironment where max_turns=2
    The environment provides a question/prompt and evaluates the response using a custom reward function.
    **It returns an empty observation.**
    """
    ...
    @staticmethod
    def from_dict(env_args: dict) -> "CritiqueEnvironment":
        reward_fn = env_args.pop("reward_fn", None)
        if "task" in env_args:
            task = env_args["task"]
        else:
            task = env_args
        return CritiqueEnvironment(task=task, reward_fn=reward_fn)
